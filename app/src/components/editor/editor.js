...
export default class Editor extends Component {
	...
	open(page, cb) {
		...
		axios
			...
			.then(DOMHelper.serialiseDOMToString)
			.then((html) => axios.post('./api/saveTempPage.php', { html }))
			.then(() => this.iframe.load('../popstr.html'))
			.then(() => axios.post('./api/deleteTempPage.php'))
			.then(() => this.enebleEditing())
	...
	async save(onSuccess, onError) {
		...
		const html = DOMHelper.serialiseDOMToString(newDom);
		await axios
	...
	enebleEditing() {
		...
		this.iframe.contentDocument.body.querySelectorAll('[editableimgid]').forEach((el) => {
			...
			const virtualEl = this.virtualDom.body.querySelector(`[editableimgid="${id}"]`);
			new EditorImages(el, virtualEl, this.isLoading, this.isLoaded, this.showNotifications);
		});
	}
    ...
    restoreBackup(e, backup) {
        ...
        UIKit.modal.confirm('Do you really want to restore the page from this backup? All unsaved data will be lost!', {labels: {ok: 'Restore', cancel: 'Cancel'}})
        .then(() => {
            this.isLoading()
            return axios
		...
		return (
			<>
				...
				<ConfirmModal
                    ...
                    text={{
                        title: 'Save',
                        descr: 'Do you really want to save the changes?',
                        btn: 'Publish'
                    }}
                />
